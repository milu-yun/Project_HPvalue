%% setting
behavior_mat = 'vHP14_rwprob06_20210727_190132';%need correction
cell_mat = '03-Aug_17_11_27.mat';
fr=30; % frame rate
...
C_d = mean(C_event(:,0.5*fr+1:3.5*fr,:),2); %cue +delay1
beforetrial = rev_index(1,5)-100;
if ismember(trial_kind,[0 1 3 4]); %rwpn
...
analysistrial= [beforetrial:beforetrial+100,aftertrial:aftertrial+100];

clear panovaset
for ii = 1:size(C_raw,1)
    [p,tbl,stats] = anovan(C_cd(analysistrial,:,ii),{cueanova(analysistrial),outanova(analysistrial)},'varnames',{'Identity','Contingency'},...
        'display','off');
    panovaset(ii,:) = p;
